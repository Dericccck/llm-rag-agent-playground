...
def qwen_llm_callable(*args, **kwargs):
    """
    使用 openai SDK 调用 Qwen 兼容接口
    """
    # 提取参数
    model = kwargs.get("model", "qwen-max")  # 可选: qwen-turbo, qwen-plus, qwen-max
    ...
    response = qwen_client.chat.completions.create(
        ...
        top_p=top_p,
        # 注意：不需要设置 api_key 或 base_url，已在 client 中定义
    )

    # 返回原生 response，Guardrails 会自动解析
    return response

hallucination_guard = AsyncGuard(name="hallucination_guard").use(ProvenanceLLM, validation_methos="full", llm_callable=qwen_llm_callable, on_fail=OnFailAction.EXCEPTION)
