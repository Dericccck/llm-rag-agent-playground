...
class SimpleVectorDB:
    ...
    def add_strings(self, strings: List[str]):
        ...
        # print("———————————————————————模型向量转换——————————————————————————")
        # for emdbedding in new_embeddings:
        #     print(emdbedding)
        # print("———————————————————————end————————————————————————————————————")
        print("文本转换向量->self.embeddings向量存储")
    ...
    def query(
        ...
        print(f"查询文本：{query_string} 转换向量：{query_embedding}")
        ...
        print(f"向量数据距离运算结果：{distances}")
        # Sort indices by distance
        sorted_indices = np.argsort(distances)

        results = []
        print(f"根据查询参数k:{k} 查询参数threshold：{threshold}进行向量数据过滤")
...
class RAGChatWidget(ChatWidget):
    ...
    def handle_submit(self, change):
        if (
            ...
            print(f"用户消息组装：{augmented_user_msg}")
            error = False
            query_messages = self.messages.copy()
            ...
            try:
                ...
                self.messages.append({"role": "assistant", "content": bot_message})
                print("augmented_user_msg bot_message添加到历史消息列表")
            except Exception as e:
                print(e)
