...
# 核心变化：从 providers.openai 路径导入
from trulens.providers.openai import OpenAI



def get_prebuilt_trulens_recorder(query_engine, app_id):
    # provider = LiteLLM(model_engine="dashscope/qwen-max")
    # 修复方案：使用 openai/ 前缀并指定 DashScope 的兼容地址
    provider = LiteLLM(
        model_engine="openai/qwen-max", 
        completion_kwargs={
...
def build_sentence_window_index(
    ...
    # 配置全局设置
    Settings.llm = llm
    Settings.embed_model = model_real_path
